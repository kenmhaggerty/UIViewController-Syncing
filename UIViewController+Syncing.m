...
//        [self.syncView setAlpha:0.0];
    ...
    else if ([AKDebugger printForMethod:METHOD_NAME logType:AKLogTypeDebug methodType:AKMethodTypeUnspecified]) NSLog(@"[INFO] Syncing has already begun for %s", __PRETTY_FUNCTION__);
}
...
- (void)cancelSyncViewWithStatus:(NSString *)status completionType:(AKSyncCompletionType)completionType actionSheet:(UIActionSheet *)actionSheet delay:(NSTimeInterval)delay completionBlock:(void (^)(BOOL))completionBlock
{
    ...
    else if ([AKDebugger printForMethod:METHOD_NAME logType:AKLogTypeDebug methodType:AKMethodTypeUnspecified]) NSLog(@"[INFO] Syncing has already ended for %s", __PRETTY_FUNCTION__);
}
